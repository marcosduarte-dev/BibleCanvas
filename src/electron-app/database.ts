...
const TABLE_NAMES = Object.keys(SCHEMA) as (keyof typeof SCHEMA)[];
 ...
 */
function validateSchema(db: SqlJsDatabase): boolean {
  try {
    const results = db.exec("SELECT name FROM sqlite_master WHERE type='table'")
    if (results.length === 0) return false

    const existingTables = new Set(results[0].values.map((row: any) => row[0] as string))
...
function initializeSchema(db: SqlJsDatabase): void {
  ...
  CANONICAL_BOOKS.forEach(book => {
    db.run(
      'INSERT OR IGNORE INTO livros (id, testamento, nome, sigla, ordem) VALUES (?, ?, ?, ?, ?)',
      [book.id, book.testamento, book.nome, book.sigla, book.id]
    )
  })
}
