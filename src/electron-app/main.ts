...
function setupIpcHandlers() {
    ...
    ipcMain.handle('db:get-versoes', () => {
        try {
            ...
            const data = results[0].values.map((row: any) =>
    ...
    ipcMain.handle('db:get-versiculos', (_event, { versao_id, livro_id, capitulo }) => {
        try {
            const results = db!.exec(
                'SELECT * FROM versiculos WHERE versao_id = ? AND livro_id = ? AND capitulo = ? ORDER BY numero',
                [versao_id, livro_id, capitulo]
            )
            if (results.length === 0) return { success: true, data: [] }
    ...
    ipcMain.handle('db:get-anotacao', (_event, { livro_id, capitulo }) => {
        try {
            const results = db!.exec(
                'SELECT * FROM anotacoes_canvas WHERE livro_id = ? AND capitulo = ?',
                [livro_id, capitulo]
            )
            ...
            const data = rowData
                ? Object.fromEntries(columns.map((col: string, i: number) => [col, rowData[i]]))
                : null
            return { success: true, data }
        } catch (error) {
    ...
    ipcMain.handle('db:save-anotacao', (_event, { livro_id, capitulo, dados_json }) => {
        try {
            db!.run(
                `INSERT INTO anotacoes_canvas (livro_id, capitulo, dados_json, ultima_atualizacao)
                VALUES (?, ?, ?, datetime('now'))
                ON CONFLICT(livro_id, capitulo)
                DO UPDATE SET dados_json = excluded.dados_json, ultima_atualizacao = datetime('now')`,
                [livro_id, capitulo, dados_json]
            )
            saveDatabase(db!)
            return { success: true }
        } catch (error) {
    ...
    ipcMain.handle('db:import-version', (_event, { nome, sigla, idioma, jsonData }) => {
        try {
            ...
            // 1. Insert version
            db!.run(
                'INSERT INTO versoes (nome, sigla, idioma) VALUES (?, ?, ?)',
                [nome, sigla, idioma || 'pt-br']
            );

            // Get the inserted version ID
            const result = db!.exec('SELECT last_insert_rowid() as id');
            const versionId = result[0].values[0][0] as number;

            // 2. Process books
            for (const book of data) {
                // Find matching canonical book either by exact sigla match, exact name match, or guess
                const canonicalBook = CANONICAL_BOOKS.find(b =>
                    b.sigla.toLowerCase() === book.abbrev.toLowerCase() ||
                ...
                if (Array.isArray(book.chapters)) {
                    book.chapters.forEach((chapterVerses: string[], chapterIndex: number) => {
                        ...
                        chapterVerses.forEach((verseText: string, verseIndex: number) => {
                            ...
                            db!.run(
                                'INSERT INTO versiculos (versao_id, livro_id, capitulo, numero, texto) VALUES (?, ?, ?, ?, ?)',
                                [versionId, canonicalBook.id, capitulo, numero, verseText]
                            );
                        });
                    });
                }
